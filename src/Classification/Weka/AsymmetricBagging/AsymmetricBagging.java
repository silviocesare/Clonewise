 ...
 */
public class AsymmetricBagging
  ...
  public void buildClassifier(Instances data) throws Exception {
    ...
    Instances positivePool = new Instances(data, 0);
    Instances negativePool = new Instances(data, 0);
    int p0 = 0;
    int p1 = 0;
    ...
    if (p1 > p0) {
      for (int i = 0; i < data.numInstances(); i++) {
        if ((int) data.instance(i).classValue() == 0) {
          positivePool.add(data.instance(i));
        } else {
          negativePool.add(data.instance(i));
        }
      }
    } else {
      for (int i = 0; i < data.numInstances(); i++) {
...
//    int bagSize = data.numInstances() * m_BagSizePercent / 100;
    int bagSize = positivePool.numInstances() * 2;
...
*/
        Instances newBagData = new Instances(positivePool, 0);
	negativePool.randomize(random);
	for (int n = 0; n < positivePool.numInstances(); n++) {
          newBagData.add(positivePool.instance(n));
        }
	for (int n = 0; n < positivePool.numInstances() && n < negativePool.numInstances(); n++) {
          newBagData.add(negativePool.instance(n));
	}
        bagData = newBagData;
      }
...
//	  double pred = m_Classifiers[j].classifyInstance(data.instance(i));
  ...
  public String getRevision() {
    return RevisionUtils.extract("$Revision: 6502 $");
  }
  ...
  public static void main(String [] argv) {
    runClassifier(new AsymmetricBagging(), argv);
  }
}
